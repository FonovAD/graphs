...
func TestUnion(t *testing.T) {
	...
	for _, edge := range expectedEdges {
		if !edgeExists(result, edge[0], edge[1]) {
...
func TestJoin(t *testing.T) {
	...
	g2 := createGraphB()
	result := g1.Join(g2)
	if !edgeExists(result, "A", "F") && !edgeExists(result, "A", "G") {
		t.Errorf("Expected join edge A-F or A-G")
	}
	if !edgeExists(result, "B", "F") && !edgeExists(result, "B", "G") {
...
func TestCheckCartesianProduct(t *testing.T) {
	g1 := createTestGraph1()
	g2 := createTestGraph2()
	expected := g1.CartesianProduct(g2)
...
func TestCheckLexicographicalProduct(t *testing.T) {
	g1 := createTestGraph1()
	g2 := createTestGraph2()
	expected := g1.LexicographicalProduct(g2)
...
func TestMinimalSpanningTree(t *testing.T) {
	...
	mst, b_ans := graph.MinimalSpanningTree()
	fmt.Println(mst)
	fmt.Println(b_ans)
	// if ans_bool == false {
	// 	t.Errorf("Expected true answer")
...
// 	// Пустой граф (нет вершин)
// 	//emptyGraph := model.Graph{}
// 	// score = checker.CheckRadiusAndDiameter(&emptyGraph, 0, 0, map[string]map[string]int{})
// 	// if score != 100 {
// 	// 	t.Errorf("CheckRadiusAndDiameter failed: expected 100 for empty graph, got %d", score)
// 	// }
...
func TestChecker_CheckAdjacentMatrix_Undirected(t *testing.T) {
	checker := taskcheck.NewChecker()
...
func TestChecker_CheckEulerGraph1(t *testing.T) {
	...
	task := model.Graph{
		...
		Edges: []model.Edge{
			...
			{Source: makeNode(3, "C"), Target: makeNode(4, "D"), Label: "3"},
			{Source: makeNode(4, "D"), Target: makeNode(5, "E"), Label: "4"},
			{Source: makeNode(5, "E"), Target: makeNode(6, "F"), Label: "5"},
			{Source: makeNode(6, "F"), Target: makeNode(7, "G"), Label: "6"},
			{Source: makeNode(7, "G"), Target: makeNode(1, "A"), Label: "7"}, // замыкает цикл
			{Source: makeNode(2, "B"), Target: makeNode(5, "E"), Label: "8"}, // дополнительное ребро для четных степеней
			{Source: makeNode(3, "C"), Target: makeNode(6, "F"), Label: "9"},
		},
	}
	...
	correctEulerPath := model.Graph{
		Edges: []model.Edge{
			...
			{Source: makeNode(5, "E"), Target: makeNode(6, "F"), Label: "5"},
			{Source: makeNode(6, "F"), Target: makeNode(7, "G"), Label: "6"},
			{Source: makeNode(7, "G"), Target: makeNode(1, "A"), Label: "7"},
			{Source: makeNode(2, "B"), Target: makeNode(5, "E"), Label: "8"},
			{Source: makeNode(3, "C"), Target: makeNode(6, "F"), Label: "9"},
		},
	}
...
func TestChecker_CheckMinPath1(t *testing.T) {
	...
	// Краевой случай: путь из вершины в саму себя
	score = checker.CheckMinPath(&task, "A", "A", 0, map[string]int{"A": 0}, &model.Graph{})
	if score != 100 {
		t.Errorf("CheckMinPath failed for path from node to itself")
	}

	// Краевой случай: нет пути между двумя вершинами (например, из G в A если ребра направленные и нет обратного пути)
	// Для неориентированного графа в нашем случае путь есть, поэтому создадим искусственный случай:
	disconnectedGraph := model.Graph{
		Nodes: []model.Node{makeNode(1, "A"), makeNode(2, "B")},
		Edges: []model.Edge{}, // Нет ребер
	}
	score = checker.CheckMinPath(&disconnectedGraph, "A", "B", -1, map[string]int{}, &model.Graph{})
	if score != 0 {
		t.Errorf("CheckMinPath failed to detect missing path")
	}
...
func TestChecker_CheckPlanarGraph(t *testing.T) {
	...
	// 2) Непланарный граф: пересекающиеся ребра (K4 с пересечением)
	nonPlanarGraph := model.Graph{
		...
		Edges: []model.Edge{
			{Id: "1", Source: makeNode(1, "A", 0, 0), Target: makeNode(3, "C", 2, 2)},
			{Id: "2", Source: makeNode(2, "B", 2, 0), Target: makeNode(4, "D", 0, 2)},
		},
	}
	...
	if score := checker.CheckPlanarGraph(&noEdgesGraph); score != 100 {
		t.Errorf("Graph without edges expected 100, got %d", score)
	}

	// 4) Граф с соседними ребрами, пересечение только в вершинах
	...
	// 5) Планарный граф: квадрат с диагоналями (K4 без пересечений)
...
// 	}
// 	ch := &checker{}
// 	correct, odd := ch.checkBinaryOperations(answer, trueAnswer)
// 	expected := Max_(0, int(math.Ceil(100.0*float64(correct-odd)/float64(len(trueAnswer.Edges)))))
// 	score := ch.CheckIntersectionGraphs(answer, graph1, graph2)
// 	if score != expected {
...
// 	correct, odd := ch.checkBinaryOperations(answer, trueAnswer)
// 	expected := Max_(0, int(math.Ceil(100.0*float64(correct-odd)/float64(len(trueAnswer.Edges)))))
// 	score := ch.CheckUnionGraphs(answer, graph1, graph2)
// 	if score != expected {
// 		t.Errorf("expected %d, got %d", expected, score)
// 	}
// }
...
// 			{Source: model.Node{Label: "A"}, Target: model.Node{Label: "B"}},
// 			{Source: model.Node{Label: "C"}, Target: model.Node{Label: "D"}},
// 			// соединение между группами
// 			{Source: model.Node{Label: "A"}, Target: model.Node{Label: "C"}},
// 			{Source: model.Node{Label: "A"}, Target: model.Node{Label: "D"}},
// 			{Source: model.Node{Label: "B"}, Target: model.Node{Label: "C"}},
// 			{Source: model.Node{Label: "B"}, Target: model.Node{Label: "D"}},
// 		},
// 	}
// 	trueAnswer := &model.Graph{
// 		Nodes: []model.Node{
...
// 	//correct, odd := ch.checkBinaryOperations(answer, trueAnswer)
// 	//expected := Max_(0, int(math.Ceil(100.0*float64(correct-odd)/float64(len(trueAnswer.Edges)))))
// 	score := ch.CheckJoinGraphs(answer, graph1, graph2)
// 	if score != expected {
// 		t.Errorf("expected %d, got %d", expected, score)
// 	}
// }
...
// 	// Вызов алгоритма
// 	result := graph.MinimumSpanningTree()

// 	// Проверка корректности
// 	ch := &checker{}
// 	correct, odd := ch.checkBinaryOperations(result, trueMST)
// 	expected := Max_(0, int(math.Ceil(100.0*float64(correct-odd)/float64(len(trueMST.Edges)))))
...
// AddNode, AddEdge, Union, Intersect, Join, CartesianProduct, TensorProduct, LexicographicalProduct, EdgeCasesEmptyGraph – остаются без изменений

func TestMinPath(t *testing.T) {
	g := createTestGraph()
	source, _ := g.FindNodeByLabel("A")
	target, _ := g.FindNodeByLabel("E")
	distance, pathMap := g.MinPath(source, target, true)
	if distance != 10 {
		t.Errorf("expected distance 10, got %d", distance)
	}
	if pathMap["A"] != 0 || pathMap["E"] != 10 {
		t.Errorf("unexpected path values: %v", pathMap)
	}
}
...
func TestMinimalSpanningTree1(t *testing.T) {
	...
	if totalWeight != 10 {
		t.Errorf("expected MST total weight 10, got %d", totalWeight)
	}
}
