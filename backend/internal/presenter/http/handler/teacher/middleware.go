...
func (h *teacherHandler) TeacherMiddleware() echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(ctx echo.Context) error {
			...
			teacher, err := h.teacherUseCase.AuthToken(ctx.Request().Context(), headerSplit[1])
			if err != nil {
			...
			ctx.Set("teacherID", teacher.TeacherID)
			ctx.Set("userID", teacher.UserID)
			return next(ctx)
		}
	}
}
